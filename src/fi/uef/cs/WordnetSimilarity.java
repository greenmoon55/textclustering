...
public class WordnetSimilarity {
	 ...
	 String dir = "D:/Past_Finland/WordNet";
...
//	String dir= directory.getAbsolutePath()+"/WordNet";
		...
		double similarityValuetemp=0;	
		 double similarityValue=0;
			...
			if (method == 1) {
				...
				similarityValuetemp = jiang.max(word1, word2, type);
			
				return similarityValuetemp;

			} else if (method== 2) {
				wu=ws.getWuAndPalmer();
				similarityValuetemp = wu.max(word1, word2, type);
				
				return similarityValuetemp;
				
			} else if (method == 3) {
				lin = ws.getLin();
				similarityValuetemp = lin.max(word1, word2, type);
			
				return similarityValuetemp;
				
			} else if (method == 4) {
				path = ws.getPath();
				similarityValuetemp = path.max(word1, word2, type);
			...
			// double similarityValue = jinag.jcn(word1, 1, word2, 1, type);

		}

		
	// get the similarity by just comparing the first meaning of two words 
	public double getWordnetSimilarity(String word1, String word2,
			String type){
	...
	/*get the similarity by just comparing the all the meaning of two words by five different measure
	 * 1. JiangAndConrath
	 * 2. Lin
	 * 3. Path
	 * 4. WuAndPalmer
	...
	public double getWordnetSimilarityByMeasure(String word1, String word2,
		...
		if (measure.trim().equalsIgnoreCase("1")) {
			...
			double similarityValue= jiangAndConrath.jcn(word1, 1, word2, 1, type);
			return similarityValue;
	...
	/*get the similarity by just comparing the first the meaning of two words by five different measure
	 * 1. JiangAndConrath
	 * 2. Lin
	 * 3. Path
	 * 4. WuAndPalmer
	...
	public <E> double getWordDistanceAll(String word1, String word2,
			String type) {
		...
		//double similarityValue = wup.max(word1, word2, type);
		double wordsimilarityValue = wup.max(word1.toString(),word2.toString(),type);
		double worddistanceValue=1-wordsimilarityValue;
		java.text.DecimalFormat df=new   java.text.DecimalFormat("#0.00");
		double distacneValue= Double.parseDouble(df.format(worddistanceValue*100));
	...
	public static HashMap<Integer, String> getWordGloss(String keyword,
		...
		String path = "D:/Past_Finland/WordNet/3.0" + File.separator + "dict";
